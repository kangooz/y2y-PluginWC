...
}(function ($) 
{
	...
	function DateTimePicker(element, options)
	{
		...
		this.settings = jQuery.extend({}, jQuery.DateTimePicker.defaults, options, jQuery.DateTimePicker.i18n[sLanguage]);
				...
				if(options === "destroy")
				{
					if(sArrDataKeys.length > 0)
					{
						for(iKey in sArrDataKeys)
						{
							...
							{
								jQuery(document).unbind("click.DateTimePicker");
								jQuery(document).unbind("keydown.DateTimePicker");
								jQuery(document).unbind("keyup.DateTimePicker");
							
								jQuery(this).children().remove();
								jQuery(this).removeData();
								jQuery(this).unbind();
								jQuery(this).removeClass("dtpicker-overlay dtpicker-mobile");
	...
	DateTimePicker.prototype = {
		...
		_addEventHandlersForInput: function()
		{
			...
			jQuery(oDTP.settings.parentElement).on ("focus", sel, {"obj": oDTP}, oDTP._inputFieldFocus);
		
			jQuery(oDTP.settings.parentElement).off("click", sel, oDTP._inputFieldClick);
			jQuery(oDTP.settings.parentElement).on ("click", sel, {"obj": oDTP}, oDTP._inputFieldClick);
		...
		getDateObjectForInputField: function(oInputField)
		{
			...
			if(jQuery.cf._isValid(oInputField))
			{
				...
				{
					dInput = oDTP.settings.parseDateTimeString.call(oDTP, sDateTime, sMode, jQuery(oInputField));
				}
		...
		_showPicker: function(oElement)
		{
			...
			if(oDTP.oData.oInputElement === null)
			{
				...
				if(oDTP.oData.bDateMode)
				{
						...
						if(jQuery(sStartEndElem).length >= 1)
						{
							...
							if(sTempDate !== "")
							{
								if(oDTP.settings.parseDateTimeString)
									dTempDate = oDTP.settings.parseDateTimeString.call(oDTP, sTempDate, sMode, jQuery(sStartEndElem));
								else
					...
					if(oDTP.settings.parseDateTimeString)
						oDTP.oData.dCurrentDate = oDTP.settings.parseDateTimeString.call(oDTP, sCurrent, sMode, jQuery(oElement));
					else
				...
				else if(oDTP.oData.bTimeMode)
				{
					...
					if(sStartEnd !== "" && (jQuery.cf._compare(sStartEnd, "start") || jQuery.cf._compare(sStartEnd, "end")) && sStartEndElem !== "")
					{
						if(jQuery(sStartEndElem).length >= 1)
						{
							...
							{
								if(oDTP.settings.parseDateTimeString)
									dTempDate = oDTP.settings.parseDateTimeString.call(oDTP, sTempTime, sMode, jQuery(sStartEndElem));
								else
				...
				else if(oDTP.oData.bDateTimeMode)
				{
					...
					if(sStartEnd !== "" && (jQuery.cf._compare(sStartEnd, "start") || jQuery.cf._compare(sStartEnd, "end")) && sStartEndElem !== "")
					{
						if(jQuery(sStartEndElem).length >= 1)
						{
							...
							{
								if(oDTP.settings.parseDateTimeString)
									dTempDateTime = oDTP.settings.parseDateTimeString.call(oDTP, sTempDateTime, sMode, jQuery(sStartEndElem));
								else
		...
		_addEventHandlersForPicker: function()
		{
			...
			{
				if(! jQuery('.dtpicker-compValue').is(':focus') && parseInt(e.keyCode ? e.keyCode : e.which) === 9)
				{
			...
			jQuery('.dtpicker-compValue').blur(function()
			{
				...
				var $oParentElem = jQuery(this).parent().parent();
				setTimeout(function()
				{
			...
			jQuery(".dtpicker-compValue").keyup(function()
			{
				var $oTextField = jQuery(this),
		...
		_setValueOfElement: function(sElemValue, $oElem)
		{
			...
			if(!jQuery.cf._isValid($oElem))
				$oElem = jQuery(oDTP.oData.oInputElement);
		
			if(jQuery.cf._compare($oElem.prop("tagName"), "INPUT"))
		...
		setLanguage: function(sLanguage)
		{
			...
			oDTP.settings = jQuery.extend({}, jQuery.DateTimePicker.defaults, oDTP.options, jQuery.DateTimePicker.i18n[sLanguage]);
